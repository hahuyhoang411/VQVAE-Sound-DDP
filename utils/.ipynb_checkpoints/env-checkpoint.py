...
def collate_multispeaker_samples(left_pad, window, right_pad, batch):
    ...
    speakers_onehot = torch.FloatTensor(np.array([x[0] for x in batch]))
...
# def collate_multispeaker_samples(left_pad, window, right_pad, batch):
#     samples = [x[1] for x in batch]
#     speakers_onehot = torch.tensor(np.array([x[0] for x in batch]), dtype=torch.bfloat16)
#     max_offsets = [x.shape[-1] - window for x in samples]
#     offsets = [np.random.randint(0, offset) for offset in max_offsets]

#     wave16 = [torch.cat([
#                   torch.zeros(left_pad, dtype=torch.bfloat16), 
#                   torch.tensor(x, dtype=torch.bfloat16),
#                   torch.zeros(right_pad, dtype=torch.bfloat16)
#               ])[offsets[i]:offsets[i] + left_pad + window + right_pad] for i, x in enumerate(samples)]
#     return speakers_onehot, torch.stack(wave16)
