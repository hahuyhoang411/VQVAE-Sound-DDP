...
def main(rank, world_size):
    ...
    if dataset_type == 'multi':
        ...
        print("Number of speakers:", dataset.num_speakers())
    ...
    model = model_fn(dataset).to(rank)
    ...
    if use_half:
        ...
        with torch.cuda.device(rank):
            optimiser = FusedAdam(model.parameters(), weight_decay=1e-2)
    else:
        optimiser = optim.AdamW(model.parameters())
    
    if args.generate:
        model.module.do_generate(paths, step, data_path, test_index, use_half=use_half, verbose=True)
    else:
        ...
        model.module.do_train(paths, dataset, optimiser, epochs=5000, batch_size=256, step=step, lr=1e-5, use_half=use_half, valid_index=test_index)
